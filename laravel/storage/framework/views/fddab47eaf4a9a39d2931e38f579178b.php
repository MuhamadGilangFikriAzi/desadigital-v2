<?php $__env->startSection("content"); ?>
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        ...
        <div class="p-4 border-b border-gray-200">
            ...
            <form action="<?php echo e(route("users.index")); ?>" method="get" enctype="multipart/form-data" id="filter">
                <div class="flex flex-wrap -mx-3">
                    <div class="w-full md:w-1/2 px-3 mb-3">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Nama</label>
                        <input type="text" name="name" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" value="<?php echo e($filter["name"]); ?>">
                    </div>
                    <div class="w-full md:w-1/2 px-3 mb-3">
            ...
            <div class="overflow-x-auto">
                <table class="w-full border-collapse bg-white rounded-lg overflow-hidden shadow-sm">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        <?php $__empty_1 = true; $__currentLoopData = $list; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $l): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <tr class="hover:bg-gray-50 transition">
                                <td class="px-4 py-3 text-center font-medium"><?php echo e($key + 1); ?></td>
                                <td class="px-4 py-3 text-center"><?php echo e($l->name); ?></td>
                                <td class="px-4 py-3 text-center">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                        <?php echo e($l->roles->pluck('name')[0] == 'Staff Desa' ? 'bg-purple-100 text-purple-800' : ($l->roles->pluck('name')[0] == 'Guest' ? 'bg-yellow-100 text-yellow-800' : 'bg-green-100 text-green-800')); ?>">
                                        <?php echo e($l->roles->pluck("name")[0]); ?>

                                    </span>
                                </td>
                                <td class="px-4 py-3 text-center"><?php echo e(date("d M Y", strtotime($l->created_at))); ?></td>
                                <td class="px-4 py-3 text-center">
                                    <div class="flex justify-center gap-2">
                                        <button class="inline-flex items-center gap-1 bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm py-1.5 px-3 rounded-lg transition btn-show"
                                            data-id="<?php echo e($l->id); ?>" data-toggle="modal" data-target="#show-user">
                                            <i class="fas fa-eye"></i> Show
                                        </button>
                                        <button class="inline-flex items-center gap-1 bg-red-50 hover:bg-red-100 text-red-600 text-sm py-1.5 px-3 rounded-lg transition btn-remove" data-id="<?php echo e($l->id); ?>">
                    ...
                    <tfoot class="bg-gray-50">
                        <tr>
                            <td colspan="4" class="px-4 py-3 text-center"><?php echo e($list->links()); ?></td>
                            <td class="px-4 py-3 text-right text-gray-500 text-sm">Total entries: <?php echo e($data); ?></td>
                        </tr>
                    </tfoot>
                </table>
            </div>
            ...
            $(document).on('click', '.btn-show', function() {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        let resp = data.data;
                        let urlImg = "<?php echo e(asset("img/ktp/")); ?>" + "/" + resp.data.ktp;
...
<?php echo $__env->make("layouts.app", array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /var/www/html/resources/views/user/list.blade.php ENDPATH**/ ?>