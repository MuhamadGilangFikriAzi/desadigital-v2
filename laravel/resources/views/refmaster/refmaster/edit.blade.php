...
@section("content")
    <div class="max-w-2xl mx-auto">
        <div class="admin-card-header">
            <h5 class="mb-0"><i class="fas fa-edit mr-2"></i> Edit Referensi</h5>
        </div>
        <div class="p-4">
            <form action="{{ route("refmaster.update", $refMaster->id) }}" method="POST">
                @csrf @method("PUT")
                <div class="mb-4">
                    ...
                    <select name="ref_master_type_code" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white @error("ref_master_type_code") border-red-500 @enderror">
                        ...
                        @foreach ($types as $type)
                            <option value="{{ $type->ref_master_type_code }}" @if(old("ref_master_type_code", $refMaster->ref_master_type_code) == $type->ref_master_type_code) selected @endif>{{ $type->ref_master_type_name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="ref_master_name" value="{{ old("ref_master_name", $refMaster->ref_master_name) }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error("ref_master_name") border-red-500 @enderror">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="ref_master_value" value="{{ old("ref_master_value", $refMaster->ref_master_value) }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
