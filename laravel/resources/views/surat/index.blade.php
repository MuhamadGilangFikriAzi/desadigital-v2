    ...
    @hasanyrole("User|Staff Desa")
        <div class="bg-white rounded-lg shadow-sm border border-gray-200">
            <div class="admin-card-header flex justify-between items-center pl-2">
                ...
                @role("User")
                    <a href="{{ route("suratcreate") }}" class="inline-flex items-center gap-1 bg-green-600 hover:bg-green-700 text-white font-medium py-2 px-4 rounded-lg transition text-sm ml-auto">
            ...
            <div class="admin-card-body">
                <form action="{{ route("surat") }}" method="get" class="flex flex-wrap -mx-3 mb-4">
                    <div class="w-full md:w-1/3 px-3 mb-3 md:mb-0">
                        <label class="block text-sm font-medium text-gray-700 mb-1">ID</label>
                        <input type="text" name="id" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" value="{{ $filter["id"] }}">
                    </div>
                    <div class="w-full md:w-1/3 px-3 mb-3 md:mb-0">
                        ...
                        <select name="template_surat_id" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white" id="type_surat">
                            ...
                            @foreach ($listTemplateSurat as $item)
                                <option value="{{ $item->id }}" @if (isset($filter["template_surat_id"]) && $filter["template_surat_id"] == $item->id) selected @endif>
                                    {{ $item->type_surat }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full border-collapse bg-white rounded-lg overflow-hidden shadow-sm">
                        ...
                        <tbody class="divide-y divide-gray-200">
                            @forelse ($list as $data)
                                <tr class="hover:bg-gray-50 transition">
                                    ...
                                    <td class="px-4 py-3 text-center">{{ $data->template_surat->type_surat }}</td>
                                    <td class="px-4 py-3 text-center">{{ date("d M Y", strtotime($data->created_at)) }}</td>
                                    @role("Staff Desa")
                                        <td class="px-4 py-3 text-center">{{ $data->user->name }}</td>
                                    @endrole
                                    <td class="px-4 py-3 text-center">{{ $data->code_surat_printed ?? "-" }}</td>
                                    <td class="px-4 py-3 text-center">{{ $data->printed_at ? date("d M Y H:i", strtotime($data->printed_at)) : "-" }}</td>
                                    <td class="px-4 py-3 text-center">
                                        <div class="flex justify-center gap-1">
                                            @role("User")
                                                @if (!$data->code_surat_printed)
                                                    <a href="{{ url("/surat/edit/" . $data->id) }}" class="inline-flex items-center gap-1 bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm py-1.5 px-3 rounded-lg transition">
                                            ...
                                            @role("Staff Desa")
                                                <button class="inline-flex items-center gap-1 bg-blue-50 hover:bg-blue-100 text-blue-600 text-sm py-1.5 px-3 rounded-lg transition btn-print"
                                                    data-id="{{ $data->id }}" data-toggle="modal" data-target="#print">
                        ...
                        <tfoot class="bg-gray-50">
                            <tr>
                                <td colspan="3" class="px-4 py-3">{{ $list->links() }}</td>
                                <td colspan="4" class="px-4 py-3 text-right text-gray-500 text-sm">Total entries: {{ $count }}</td>
                            </tr>
                        </tfoot>
                    </table>
                </div>
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            ...
            <div class="relative inline-block bg-white rounded-xl shadow-2xl max-w-4xl w-full mx-4 z-10">
                ...
                <div class="px-6 py-4" id="print-body">
                    <form id="generatePDF" action="{{ route("geerateSuratPDF") }}" method="post" enctype="multipart/form-data">
                        @csrf
                        <div class="mb-4">
    ...
    <div class="fixed inset-0 z-50 hidden overflow-y-auto" id="report-modal" x-data="{ open: false }" x-show="open" x-transition>
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            ...
            <div class="relative inline-block bg-white rounded-xl shadow-2xl max-w-2xl w-full mx-4 z-10">
                ...
                <div class="px-6 py-4">
                    <form id="generateReport" action="{{ route("generateReportExcel") }}" method="post" enctype="multipart/form-data">
                        @csrf
                        <div class="mb-4">
                            <label class="block text-sm font-medium text-gray-700 mb-1">Jenis Surat</label>
