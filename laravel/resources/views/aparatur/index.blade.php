...
@section("content")
    ...
    <div class="admin-card-body">
        <div class="flex justify-between items-center mb-4">
            ...
            <a href="{{ route("aparatur.create") }}" class="inline-flex items-center gap-1 bg-green-600 hover:bg-green-700 text-white font-medium py-2 px-4 rounded-lg transition text-sm">
                ...
                <tbody class="divide-y divide-gray-200">
                    @foreach ($aparatur as $data)
                        <tr class="hover:bg-gray-50 transition">
                            <td class="px-4 py-3">
                                @if ($data->photo)
                                    <img src="{{ asset("img/aparatur/img/" . $data->photo) }}" class="rounded-full w-12 h-12 object-cover"
                                        alt="Foto" onclick="showImageModal('{{ asset("img/aparatur/img/" . $data->photo) }}')">
                                @else
                                    <img src="{{ asset("img/default-user.png") }}" class="rounded-full w-12 h-12 object-cover" alt="Default Foto">
                                @endif
                            </td>
                            <td class="px-4 py-3 font-medium text-gray-900">{{ $data->name }}</td>
                            <td class="px-4 py-3 text-gray-600">{{ $data->position }}</td>
                            <td class="px-4 py-3">
                                @if ($data->is_active)
                            ...
                            <td class="px-4 py-3 text-center">
                                <div class="flex justify-center gap-2">
                                    <a href="{{ route("aparatur.edit", $data->id) }}" class="inline-flex items-center gap-1 bg-blue-500 hover:bg-blue-600 text-white text-sm py-1.5 px-3 rounded-lg transition">
                                        <i class="fas fa-edit"></i> Edit
                                    </a>
                                    <form action="{{ route("aparatur.destroy", $data->id) }}" method="POST" class="inline">
                                        @csrf
                                        @method("DELETE")
