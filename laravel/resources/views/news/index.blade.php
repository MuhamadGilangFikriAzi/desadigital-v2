...
@section("content")
    ...
    <div class="mb-4 flex flex-wrap justify-between items-center gap-3 px-3 pt-3">
        <a href="{{ route("news.create") }}" class="inline-flex items-center gap-1 bg-green-600 hover:bg-green-700 text-white font-medium py-2 px-4 rounded-lg transition text-sm">
    ...
    @forelse ($news as $item)
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 mb-4 hover:shadow-md transition">
            <div class="p-5">
                ...
                <p class="text-gray-600 text-sm mt-2 leading-relaxed">{!! Str::limit($item->content, 250) !!}</p>

                @if ($item->image)
                    <img src="{{ asset("storage/news/" . $item->image) }}" width="100" class="mt-2 max-w-full h-auto rounded">
                @endif

                <div class="flex justify-between items-center mt-4 pt-3 border-t border-gray-100">
                    <small class="text-gray-500">Penulis: {{ $item->author }}</small>
                    <div class="flex gap-2">
                        <a href="{{ route("news.show", $item->id) }}" class="inline-flex items-center gap-1 bg-blue-50 hover:bg-blue-100 text-blue-600 text-sm py-1.5 px-3 rounded-lg transition">
                            <i class="fas fa-eye"></i> Lihat
                        </a>
                        <a href="{{ route("news.edit", $item->id) }}" class="inline-flex items-center gap-1 bg-yellow-50 hover:bg-yellow-100 text-yellow-600 text-sm py-1.5 px-3 rounded-lg transition">
