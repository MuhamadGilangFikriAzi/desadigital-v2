...
let C='';
function sc(h){(h['set-cookie']||[]).forEach(c=>{const e=c.indexOf('='),s=c.indexOf(';'),n=c.substring(0,e),v=c.substring(e+1,s>0?s:c.length);const rx=new RegExp('(^|;\\s*)'+n.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')+'=[^;]*');C.match(rx)?C=C.replace(rx,'$1'+n+'='+v):C+=(C?'; ':'')+n+'='+v})}
function go(m,p,h,b){return new Promise(r=>{const o={hostname:B,path:p,method:m,headers:{'Cookie':C,...(h||{})},rejectUnauthorized:false};const q=https.request(o,res=>{let d='';sc(res.headers);const hdrs={};Object.keys(res.headers).forEach(k=>hdrs[k]=res.headers[k]);res.on('data',c=>d+=c);res.on('end',()=>r({s:res.statusCode,b:d,h:hdrs}))});q.on('error',e=>r({s:0,b:e.message,h:{}}));if(b)q.write(b);q.end()})}
async function f(){
  ...
  const ps=process.argv[2] ? process.argv[2].split(',') : ['/admin/home'];
  let r='';
  for(const p of ps){const x=await go('GET',p,{});const t=x.b.match(/<title[^>]*>([^<]+)<\/title>/);r+=p+':'+x.s+(x.s===200?'':' FAIL')+' '+(t?t[1].substring(0,25):'')+'\n'}
  console.log(r.trim());
}
f();
